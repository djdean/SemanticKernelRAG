...
async def main():
    ...
    compliance_cheking_agent = ChatCompletionAgent(
        ...
        instructions=f"""
            ...
            search for relevant context with the provided list of information and use that context to determine if the post is compliant or not. Reply with 
             '{finished_keyword}'. Always make sure to reply to with '{finished_keyword}' at the end of your response.
            """,
        arguments=KernelArguments(settings=execution_settings),
    ...
    selection_function = KernelFunctionFromPrompt(
        ...
        prompt=f"""
            ...
            - If it was just editor's turn, it is checker's turn.
            RESPONSE:
            {{{{$lastmessage}}}}
            """
        )
        ...
        try:
            async for response in chat.invoke():
                if response is None or not response.name:
                    continue
                elif more_info_keyword in response.content.lower():
                    print()
                    print(f"# {response.name} Agent: {response.content}")
                    continue
                elif finished_keyword in response.content.lower():
                    print()
                    print(f"# {response.name} Agent: {response.content}")
                    continue
                else:
