...
def init_semantic_kernel(app):
    ...
    search_agent = ChatCompletionAgent(
        ...
        name="AISearchAgent",
        instructions=f"""
            You are an AI agent that is responsible for checking if posts are compliant. The tools you are provided with allow you to search for specific information related to whether the post is compliant or not.
            """,
...
def handle_question(question, app, history):
    ...
    if not st.session_state["semantic_kernel"]:   
        handle_question_native(question_with_prefix, app, history)
    else:
        handle_question_semantic_kernel(question_with_prefix)
...
async def get_answer_from_agents(question):
    ...
    async for agent_response in st.session_state["agent"].invoke(
        messages=question, 
        thread=st.session_state["history"], 
        arguments=None):
        with st.chat_message("assistant"):
        ...
        response_message = {"role": "assistant", "content": response}
        st.session_state["messages"].append(response_message)
        st.session_state["history"] = agent_response.thread
